...
studentsNum <- studentsNum[featureSubset]
trainingNum <- trainingNum[featureSubset]
testingNum <- testingNum[featureSubset]
students <- students[featureSubset]
training <- training[featureSubset]
testing <- testing[featureSubset]
studentsFactors <- studentsFactors[featureSubset]
trainingFactors <- trainingFactors[featureSubset]
testingFactors <- testingFactors[featureSubset]
...
accCB <- mean(predictions.mixture==testing$AcadPerformance)
...
clusters <- hclust(dist(students[, 1:6]), method = 'centroid')
plot(clusters)
numberClusters <- 2
...
clusters <- hclust(dist(students[, 1:6]))
plot(clusters)
numberClusters <- 5
clusterCut <- cutree(clusters, numberClusters)
clusterTable <- as.data.frame(table(clusterCut, students$AcadPerformance))
...
globalWeightedPredictions <- rowMeans(cbind(ruleInductionFitProb,classTreeFitProb,svmFitProb,logRegFitProb,descreteBayesFitProb,continuousBayesFitProb))
